...
class AdTest < Test::Unit::TestCase
  def test_should_initialize_from_document_ad_node
    ...
    assert ad.is_a?(VAST::Ad), 'Ad should be kind of Ad'
  end
  ...
  def test_should_initialize_inline_ad
    ...
    ad_node = document.root.xpath('.//Ad').first
    ad = VAST::Ad.create(ad_node)
    assert ad.is_a?(VAST::InlineAd), 'Ad should be kind of InLineAd'
  end
  ...
  def test_should_initialize_wrapper_ad
    ...
    ad_node = document.root.xpath('.//Ad').first
    ad = VAST::Ad.create(ad_node)
    assert ad.is_a?(VAST::WrapperAd), 'Ad should be kind of WrapperAd'
  end

  def test_should_raise_error_if_no_ad_type_specified
    filename = example_file('invalid_document_with_missing_ad_type.xml')
    document = VAST::Document.parse(example_file(filename))

    ad_node = document.root.xpath('.//Ad').first
    assert_raise VAST::InvalidAdError, 'should raise error' do
      VAST::Ad.create(ad_node)
    end
  end
  ...
  def test_ad_should_know_linear_creatives
    ...
    ad.linear_creatives.each do |creative|
      assert creative.is_a?(VAST::LinearCreative)
    end
  end
  ...
  def test_ad_should_know_non_linear_creatives
    ...
    ad.non_linear_creatives.each do |creative|
      assert creative.is_a?(VAST::NonLinearCreative)
    end
  end
  ...
  def test_ad_should_know_companion_creatives
    ...
    ad.companion_creatives.each do |creative|
      assert creative.is_a?(VAST::CompanionCreative)
    end
  end
  ...
  def test_ad_should_know_its_impressions
    ...
    document = VAST::Document.parse!(example_file(filename))
    impressions = document.inline_ads.first.impressions

    assert_equal 2, impressions.count
    assert_equal 'http://myTrackingURL/impression', impressions.first.to_s
    assert_equal 'http://myTrackingURL/anotherImpression', impressions.last.to_s
  end
  ...
  def test_extensions
    ...
    ad.extensions.each do |extension|
      assert extension.is_a?(VAST::Extension)
    end
  end
end
