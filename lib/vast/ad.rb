...
module VAST
  # Contains some combination of video, companions, and non-linear units for a
  # single advertiser.
  #
  # A single VAST response may include multiple Ads from multiple advertisers.
  # It will be up to the Video Player to determine the order, timing, placement,
  # etc for the multiple ads. However, the player should generally respect the
  # sequential order of the Ad elements within a VAST response.
  #
  # The VAST response does not contain information on the placement or timing of
  # each ad. It is up to the Video Player to determine the optimal inclusion
  # points of the ads.
  #
  # Can either be a InlineAd, meaning it contains all the elements necessary to
  # display the visual experience, or a WrapperAd, which points to a downstream
  # VAST document that must be requested from another server.
  #
  # An Ad may include one or more pieces of creative that are part of a single
  # execution. For example, an Ad may include a linear video element with a set
  # of companion banners; this would be reflected by two Creative elements, one
  # LinearCreative and one CompanionCreative.
  class Ad < Element
    ...
    def ad_system
      ...
      raise(InvalidAdError, 'missing AdSystem node in Ad') unless ad_system_node
    ...
    # This is a convenience method for when only the first piece of linear
    # creative is needed. It's common for an ad to contain only one piece of
    # linear creative.
    def linear_creative
    ...
    def non_linear_creatives
      source_node.xpath('.//Creative/NonLinearAds/NonLinear')
                 .to_a.collect do |node|
    ...
    def companion_creatives
      source_node.xpath('.//Creative/CompanionAds/Companion')
                 .to_a.collect do |node|
